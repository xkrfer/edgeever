...
The setup API call itself needs a user-scoped token with **Workers Builds Configuration: Edit** and **Workers Scripts: Read**. Put it in `EDGE_EVER_BUILDS_API_TOKEN` (or reuse `CLOUDFLARE_API_TOKEN` if it has those permissions). This token is never uploaded to the Worker or Cloudflare Builds.
